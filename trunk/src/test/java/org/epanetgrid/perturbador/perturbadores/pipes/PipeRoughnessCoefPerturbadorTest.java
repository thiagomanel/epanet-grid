 ...
 * since 25/06/2007
 */
public class PipeRoughnessCoefPerturbadorTest extends TestCase {

	/*
	 * Test method for 'org.epanetgrid.perturbador.perturbadores.pipes.PipeRoughnessCoefPerturbador.disturb(IPipe)'
	 */
	public void testDisturbIPipe() {
		...
		IPipe pipePert = new PipeRoughnessCoefPerturbador(basePipe.label(), newValue).disturb(basePipe);
		...
		assertEquals(Measure.valueOf(newValue, VolumetricFlowRate.SI_UNIT).getEstimatedValue(), 
				pipePert.getRoughnessCoefficient().getEstimatedValue());
	}
