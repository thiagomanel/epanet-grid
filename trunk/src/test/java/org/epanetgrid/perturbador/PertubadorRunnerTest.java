...
public class PertubadorRunnerTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		basePipe = (IPipe) network.getElemento("P1");
	}
	...
	public void testGetMalhaPerturbadas() {
		...
		for (NetWork network : malhasPerturbadas) {
			pipesList.add(network.getElemento(basePipe.label()));
		}
		
		assertEquals(4, pipesList.size());
		...
		assertTrue( ((IPipe)pipesList.get(0)).getLength().approximates(Measure.valueOf(1, Length.SI_UNIT)) );
		assertTrue( ((IPipe)pipesList.get(1)).getLength().approximates(Measure.valueOf(2, Length.SI_UNIT)) );
		assertTrue( ((IPipe)pipesList.get(2)).getLength().approximates(Measure.valueOf(3, Length.SI_UNIT)) );
	}
