...
public class DefaultOutPutRelatorioTest extends TestCase {
	...
	public final void testBuildDefaultOutPutRelatorio() throws FileNotFoundException {
		try {
			new DefaultOutPutRelatorio.Builder().build(null);
			fail();
	...
	public final void testPressaoMaximaMinimaNode() throws IOException {
		...
		EasyMock.expect(doc1.getSource()).andReturn("T1                  42.77      1.36     15.62").anyTimes();
		EasyMock.expect(doc2.getSource()).andReturn("T4                  48.55      0.99      6.66").anyTimes();
		...
		DefaultOutPutRelatorio outRel = new DefaultOutPutRelatorio.Builder().setPressaoMatcher(matcher).
										build(linedtxtBuilder);
		
		assertEquals("T4", outRel.pressaoMinimaNode().getNodeName());
		assertEquals(6.66, outRel.pressaoMinimaNode().getPressao().getEstimatedValue(), 1e-5);
		
		assertEquals("T1", outRel.pressaoMaximaNode().getNodeName());
		assertEquals(15.62, outRel.pressaoMaximaNode().getPressao().getEstimatedValue(), 1e-5);
	}
	 ...
	 * @throws IOException 
	 */
	public final void testPressaoMaximaNode() throws IOException {
		...
		DefaultOutPutRelatorio outRel = new DefaultOutPutRelatorio.Builder().setVelocidadeMatcher(matcher).
										build(linedtxtBuilder);
		
		assertEquals("T4", outRel.velocidadeMinimaNode().getNodeName());
		assertEquals(0.99, outRel.velocidadeMinimaNode().getVelocidade().getEstimatedValue(), 1e-5);
		
		assertEquals("T1", outRel.velocidadeMaximaNode().getNodeName());
		assertEquals(1.36, outRel.velocidadeMaximaNode().getVelocidade().getEstimatedValue(), 1e-5);
