 ...
 * since 25/06/2007
 */
public class TankInitialWaterLevelPerturbadorTest extends TestCase {

	/*
	 * Test method for 'org.epanetgrid.perturbador.perturbadores.tank.TankInitialWaterLevelPerturbador.disturb(DefaultTank)'
	 */
	public void testDisturbDefaultTank() {
		...
		ITank tankPert = new TankInitialWaterLevelPerturbador(baseTank.label(), newValue).disturb(baseTank);
		...
		assertEquals(Measure.valueOf(newValue, Length.SI_UNIT).getEstimatedValue(), 
				tankPert.getInitialWaterLevel().getEstimatedValue());
	}
